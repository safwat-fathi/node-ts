...
class CrudController<T, S extends CrudService<T>> {
  ...
  async read() {
    return asyncHandler(async (req, res, next) => {
      ...
      if (!result) {
        return next(new HttpError(404, ITEM_NOT_FOUND));
      }

      res.status(200).json({ success: true, message: "Item found", data: result });
    });
  }
  ...
  async update() {
    return asyncHandler(async (req, res, next) => {
      ...
      res.status(200).json({ success: true, message: "Item updated", data: result });
    });
  }
  ...
  async list() {
    return asyncHandler(async (req, res) => {
      ...
      res.status(200).json({ success: true, message: "Items found", data: result });
    });
  }
}
